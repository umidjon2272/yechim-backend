...
export function customerScopeWhere(user: any) {
  ...
  if (user?.customerScope === 'ALL') return {};
  if (user?.customerScope === 'GROUPS') {
    const legacyGroupIds = (user?.allowedCustomerGroups || []).map((item: any) => item.groupId).filter(Boolean);
    return legacyGroupIds.length ? { groups: { some: { id: { in: legacyGroupIds } } } } : { id: '__no_allowed_group__' };
  }
  if (user?.customerScope === 'OWN') return { assignedEmployeeId: user?.id };
  if (role === 'EMPLOYEE') {
